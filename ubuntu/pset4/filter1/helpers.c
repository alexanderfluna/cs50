...
{
    //use of temporary array to swap values
    int temp[3];
    for (int j = 0; j < height; j++)
    {
...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < width; i++)
    {
        for (int j = 0; j < height; j++)
        {
            ...
            //sums values of the pixel and 8 neighboring ones, skips
            //iteration if it goes outside the image
            for (int k = -1; k < 2; k++)
            {
            ...
            //averages the sum to make image blurry
